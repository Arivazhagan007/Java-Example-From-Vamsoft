...
public class ViewEmpById implements Command{
	...
	public String execute(HttpServletRequest request,
		...
		emp=viewEmpById(emp,request);
		logger.debug(CommandUtil.AFTERVIEWBYID);
		session.setAttribute(CommandUtil.SVEMPBYID, emp);
		if(emp.getEmpName()!=null){
			return CommandUtil.SUCCESS;
		}
		else{
			request.setAttribute(CommandUtil.VEI_ERROR, CommandUtil.VEMPI_ERROR);
			return CommandUtil.FAILURE;
		}
	}

	private Employee viewEmpById(Employee emp, HttpServletRequest request) {
		ResultSet rs=null;
