...
public class EnableEmployee implements Command{
	...
	public String execute(HttpServletRequest request,
		...
		int rows=enableEmployee(empId,request);
		if(rows>0){
			Employee emp=new Employee(empId);
			emp=getEmpById(emp,request);
			logger.debug(CommandUtil.AFTERENABLE);
			session.setAttribute(CommandUtil.SEEMPBYID, emp);
			if(emp!=null){
				return CommandUtil.SUCCESS;
			}
			else{
				request.setAttribute(CommandUtil.ENABLEERROR, CommandUtil.VEMPI_ERROR);
	...
	private int enableEmployee(int empId, HttpServletRequest request) {
		int rows=0;
		PreparedStatement statement=null;
		...
		try {
			statement=connection.prepareStatement(SQLHolder.ENABLEEMPQUERY);
