...
public class ShowIssueDetails implements Command{
	...
	public String execute(HttpServletRequest request,
		...
		List<IssueDetails> list = getIssueHistory(custId,request);

		String issueHistorySubject=null;

		for (IssueDetails issueDetails2 : list) {

			issueHistorySubject = issueDetails2.getIssueSubject();
		}

		Object count = session.getAttribute(CommandUtil.SCUSTDETAILS);
		System.out.println(count);
		
		if(issueHistorySubject !=null){
			
			session.setAttribute(CommandUtil.ISSUE_HISTORY, list);
			...
			request.setAttribute(CommandUtil.FAILURE_ALL_ISSUE, CommandUtil.FAILURE_ALL_ISSUE_MSG);
			session.removeAttribute(CommandUtil.FAILURE_PENDING_ISSUE);
			session.removeAttribute(CommandUtil.FAILURE_SOLVED_ISSUE);
	...
	private List<IssueDetails> getIssueHistory(int custId,
		...
		try {
			...
			statement=connection.prepareStatement(SQLHolder.GETISSUEHISTORYQRY);
