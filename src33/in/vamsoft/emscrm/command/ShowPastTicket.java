...
public class ShowPastTicket implements Command{
	...
	public String execute(HttpServletRequest request,
		...
		List<TicketDetails> list = getPastTicketDetails(custId,request);
		...
		if(ticketLocation !=null){
			...
			session.setAttribute(CommandUtil.PAST_TICKET, list);
			...
			request.setAttribute(CommandUtil.FAILURE_PAST_TICKET, CommandUtil.FAILURE_PAST_TICKET_MSG);
			session.removeAttribute(CommandUtil.FAILURE_FUTURE_TICKET);
			session.removeAttribute(CommandUtil.FAILURE_ALL_TICKET);
	...
	private List<TicketDetails> getPastTicketDetails(int custId,
		...
		try {
			...
			statement=connection.prepareStatement(SQLHolder.GETPASTTICKETQRY);
